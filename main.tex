...
\level{Modeling the Suspicion Counter} \label{ModelingSuspicionCounter}
  ...
  \begin{figure}[t]
    \centering
    \includegraphics[width=1.0\linewidth, angle=0]{Graphs/counter_theoretical_fa_vs_thresh.png}
...
\level{Performance of the Complete Method} \label{PerformanceEvaluationCompleteMethod}
  ...
  In both cases, the maximal observed detection time rises faster than lower bound. For rogue transmitter detection, the maximal time is of the same order of magnitude of as the lower bound, while for the case of rogue receiver detection, the maximal time is an order of magnitude higher. The difference is due to the difference in misdetection rates for a single word, measured in section \ref{PerformanceEvaluationSingleWord}. The difference between the slopes is predicted by the Markov analysis. Figure \ref{fig:CounterTheoreticalTime} shows that the higher the misdetection rate of a single word, the detection time rises more rapidly with the increment of $T_{suspicion}$.
